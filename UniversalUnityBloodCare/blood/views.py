...
def admin_dashboard_view(request):
    ...
    total_unit = models.Stock.objects.aggregate(Sum('unit'))
    
    # Retrieve blood bank data
    blood_bank_data = {
        "O+ve Blood Users": models.Stock.objects.get(bloodgroup="O+").unit,
        "O-ve Blood Users": models.Stock.objects.get(bloodgroup="O-").unit,
        "AB+ve Blood Users": models.Stock.objects.get(bloodgroup="AB+").unit,
        "AB-ve Blood Users": models.Stock.objects.get(bloodgroup="AB-").unit,
        "A+ve Blood Users": models.Stock.objects.get(bloodgroup="A+").unit,
        "A-ve Blood Users": models.Stock.objects.get(bloodgroup="A-").unit,
        "B+ve Blood Users": models.Stock.objects.get(bloodgroup="B+").unit,
        "B-ve Blood Users": models.Stock.objects.get(bloodgroup="B-").unit
    }
    
    # Count total donors, total blood units, total requests, and total approved requests
    total_donors = Donor.objects.all().count()
    total_blood_unit = total_unit['unit__sum']
    total_request = models.BloodRequest.objects.all().count()
    total_approved_request = models.BloodRequest.objects.filter(status='Approved').count()
    
    # Create context dictionary
    context = {
        'A1': blood_bank_data["A+ve Blood Users"],
        'A2': blood_bank_data["A-ve Blood Users"],
        'B1': blood_bank_data["B+ve Blood Users"],
        'B2': blood_bank_data["B-ve Blood Users"],
        'AB1': blood_bank_data["AB+ve Blood Users"],
        'AB2': blood_bank_data["AB-ve Blood Users"],
        'O1': blood_bank_data["O+ve Blood Users"],
        'O2': blood_bank_data["O-ve Blood Users"],
        'totaldonors': total_donors,
        'totalbloodunit': total_blood_unit,
        'totalrequest': total_request,
        'totalapprovedrequest': total_approved_request
    }
    
    # Update JSON file with blood bank data
    update_json_file('admin-bloodbankdata.json', blood_bank_data)
    
    # Render admin dashboard template with context data
